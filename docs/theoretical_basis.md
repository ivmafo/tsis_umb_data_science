...
Para determinar si un vuelo (punto $P$) se encuentra dentro de un sector ATC (polígono $S$), utilizamos el algoritmo de **Ray Casting** (o "Even-Odd Rule").
...
El punto $P$ está dentro de $S$ si y solo si el rayo $R$ intersecta los bordes de $S$ un número **impar** de veces.
...
La capacidad teórica de un sector de control se deriva de la metodología definida en la **Circular Reglamentaria 006** de la Aeronáutica Civil.
...
A partir de la ecuación fundamental de flujo:

$$
Flujo = \frac{\text{Densidad}}{\text{Tiempo de Residencia}}
$$

Se introduce el factor de utilización máxima ($U$) y el buffer de seguridad ($B$), resultando en:
...
Donde:
*   $U \in [0, 1]$: Factor de eficiencia máxima (típicamente 0.80).
*   $B \in [0, \infty)$: Margen de buffer (típicamente 0.10).
*   $t_{occ}$: Tiempo promedio de ocupación en segundos.

### 2.2 Cálculo Ponderado de $t_{occ}$

El tiempo de ocupación no es constante; varía según la trayectoria. Se calcula como la media aritmética de los tiempos de tránsito de todas las aeronaves $N$ en una muestra histórica:
...
Para proyectar el crecimiento del tráfico a largo plazo, se utiliza un modelo de Regresión Lineal Simple, asumiendo una relación lineal entre el tiempo ($x$) y la cantidad de vuelos ($y$).
...
Los parámetros $\hat{\beta}_0$ y $\hat{\beta}_1$ se estiman minimizando la Suma de los Errores Cuadráticos (SSE):
...
Para capturar la estacionalidad compleja y los patrones no lineales del tráfico diario, se emplea un ensamble de árboles de decisión (*Random Forest Regressor*).
...
Para regresión, la métrica de impureza es el Error Cuadrático Medio (MSE):
...
El Random Forest promedia las predicciones de $K$ árboles independientes ($T_k$), entrenados sobre subconjuntos aleatorios de datos (Bootstrap).

$$
\hat{y} = \frac{1}{K} \sum_{k=1}^{K} T_k(x)
$$

Esto reduce la varianza del modelo, mitigando el sobreajuste (overfitting) típico de los árboles individuales.

### 4.3 Intervalos de Confianza (Quantile Regression Forest)

Para estimar la incertidumbre ($\hat{y} \pm \delta$), no solo predecimos la media, sino la distribución condicional completa. Aproximamos los cuantiles $q_{\alpha}$ (ej. coincidencia del 95%):
...
*   **Aerocivil**. (2015). *Circular Reglamentaria 006: Metodologías para el cálculo de capacidad en sectores ATC*. Unidad Administrativa Especial de Aeronáutica Civil.
