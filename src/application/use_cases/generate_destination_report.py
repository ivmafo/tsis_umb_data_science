...
class GenerateDestinationReport:
    def __init__(self, db_path: str = "data/metrics.duckdb"):
        self.db_path = db_path

    def _get_filter_summary(self, filters: Dict[str, Any]) -> Dict[str, str]:
        """
        Generates a human-readable summary of the applied filters.
        Returns "Todos" if a filter is not applied.
        """
        summary = {}
    ...
    def _get_data(self, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            query = "SELECT destino, COUNT(*) as count FROM flights WHERE 1=1"
            params = []
            ...
            return [{'destino': r[0], 'count': r[1]} for r in results]
    ...
    def _generate_chart(self, data: List[Dict[str, Any]]) -> io.BytesIO:
        """Generates a Treemap of the Top 10 destinations."""
        if not data:
            return None
        ...
        try:
            ...
            plt.axis('off')
            plt.title("Top 10 Destinos", fontsize=16, fontweight="bold")
        except Exception as e:
            # Fallback to bar chart if squarify fails unexpectedly
            print(f"Squarify error: {e}, using Bar Chart")
            plt.clf()
            names = [x['destino'] for x in sorted_data]
            plt.barh(names, sizes, color='#10b981')
            plt.xlabel('Vuelos')
    ...
    def generate_excel(self, filters: Dict[str, Any]) -> io.BytesIO:
        ...
        if not df.empty:
            ...
            df.rename(columns={'destino': 'Código Destino', 'count': 'Total Vuelos', 'percent': '% del Total'}, inplace=True)
    ...
    def generate_pdf(self, filters: Dict[str, Any]) -> io.BytesIO:
        ...
        elements.append(Paragraph("Reporte Ejecutivo: Distribución de Vuelos por Destino", title_style))
        ...
        if data:
            # Headers
            table_data = [['#', 'Código Destino', 'Cantidad de Vuelos', '% del Total']]
            ...
            table.setStyle(TableStyle([
                ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#10b981')), # Emerald header to match frontend
