...
class GetRegionStats:
    ...
    def execute(self, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            # Query to get count by Region and Origin
            # Structure: Region -> Origin -> Count
            
            query = """
                SELECT 
                    r.name as region_name,
                    f.origen as origin_code,
                    COUNT(*) as value
                FROM flights f
                JOIN region_airports ra ON f.origen = ra.icao_code
            ...
            query += """
                GROUP BY region_name, origin_code
            ...
            # Series: [{ name: 'Region', data: [{ x: 'Origin', y: 100 }, ...] }]
            ...
            for row in results:
                ...
                regions_map[region].append({'x': origin, 'y': count})
            ...
            # Sort regions by total volume for better visualization?
            output.sort(key=lambda s: sum(d['y'] for d in s['data']), reverse=True)
            
            return output

        except Exception as e:
            print(f"ERROR in GetRegionStats: {e}")
            raise e
        finally:
            conn.close()
